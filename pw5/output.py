...
def show_GPA():
    ...
    for Student in npstudentlist:
        if (len(Student.get_mark())==len(npcourselist)):
            print(f"{Student.get_id()} {Student.get_name()} DoB: {Student.get_DoB()} GPA: {Student.get_GPA()}")
        else:
            print(f"{Student.get_id()} {Student.get_name()} DoB: {Student.get_DoB()} GPA: {Student.get_GPA()} (Some marks of this student haven't been inputted)")
